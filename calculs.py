    ...
    for key in dict_list:
        if dict_list[key] - (dict_result[key] * quotien_elec) > i:
            ...
            if dict_list[key] > dict_list[tamp_key]:
                ...
                tamp = input("Qui est le plus âgé entre le candidat numéro " + str(
                    dict_result[key] + 1) + " de la liste " + key + " et le candidat numéro " + str(dict_result[
                                                                                                        tamp_key] + 1) + " de la liste " + tamp_key + " ? Tapez '1' pour " + key + " ou '2' pour " + tamp_key + '\n')
                tamp = int(tamp)
                while (tamp != 1 and tamp != 2):
                    tamp = input("Erreur de saisie .. Merci de recommencer : Qui est le plus âgé entre le candidat "
                                 "numéro " + str(
                        dict_result[key] + 1) + " de la liste " + key + " et le candidat numéro "
                                                                        "" + str(dict_result[
                                                                                     tamp_key] + 1) + " de la liste " + tamp_key + " ? Tapez '1' pour " + key + " ou '2' pour " + tamp_key + '\n')
                if tamp == 1:
                    i = dict_list[key] - (dict_result[key] * quotien_elec)
